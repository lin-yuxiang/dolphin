...
class ClassifierHead3D(base.BaseModelModule):
...
#         self.fc = nn.Linear(self.in_channels, self.num_classes)
        ...
        elif self.spatial_type == 'avg3d':
            self.avg_pool = nn.AdaptiveAvgPool3d((14, 1, 1))
        else:
            self.avg_pool = None
...
#                 mask[idx, :, ymin: ymin + h, xmin: xmin + w] = 1
    ...
    def forward(self, x, gt_bbox, img_meta):
        if self.with_mask:
            x = self.mask_out(x, gt_bbox, img_meta)
        x = self.avg_pool(x)
        if self.with_3d_conv:
            x = self.conv_last(x)
        x = x.view(x.size(0), -1)
