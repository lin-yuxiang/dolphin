...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, temp_feat, shared_filter, buffer_size=30):
        ...
        if len(stracks) > 0:
            ...
            for i, st in enumerate(stracks):
                if st.state != TrackState.Tracked:
                    multi_mean[i][7] = 0
            multi_mean, multi_covariance = self.shared_filter.multi_predict(
                multi_mean, multi_covariance)
            for i, (mean, cov) in enumerate(zip(multi_mean, multi_covariance)):
                stracks[i].mean = mean
                stracks[i].covariance = cov

    def activate(self, filter, frame_id):
        """Start a new tracklet"""
        self.filter = filter
        self.track_id = self.next_id()
        self.mean, self.covariance = self.filter.initiate(
