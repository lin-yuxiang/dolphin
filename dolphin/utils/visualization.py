...
def plot_tracking(image, 
    ...
    cv2.putText(
        im, 'frame: %d fps: %.2f num: %d' % (frame_id, fps, len(tlwhs)), 
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        cv2.putText(
            im, id_text, (intbox[0], intbox[1] + 30), cv2.FONT_HERSHEY_PLAIN, 
    ...
    for i, det in enumerate(tlbrs):
        ...
        if len(det) >= 7:
            ...
            if ids is not None:
                ...
                cv2.putText(
                    im, text, (x1, y1 + 30), cv2.FONT_HERSHEY_PLAIN, text_scale, 
                    (0, 255, 255), thickness=thickness)
            else:
